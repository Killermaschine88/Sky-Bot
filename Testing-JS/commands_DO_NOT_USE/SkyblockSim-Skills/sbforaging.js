...
module.exports = {
	...
	async execute(client, message, args, mclient) {
		...
		await collection.updateOne({ _id: message.author.id }, { $inc: { foraging: earnedxp } }, { upsert: true });

		const finished = new Discord.MessageEmbed()
			.setColor('90EE90')
			.setDescription(
				`Finished cutting Trees and earned <:foraging:852069714447695872> ${earnedxp} Foraging XP.`
			);
