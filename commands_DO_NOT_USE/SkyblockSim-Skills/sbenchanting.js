...
module.exports = {
	...
	async execute(client, message, args, mclient) {
		...
		await collection.updateOne({ _id: message.author.id }, { $inc: { enchanting: earnedxp } }, { upsert: true });

		const finished = new Discord.MessageEmbed()
			.setColor('90EE90')
			.setDescription(
				`Finished enchanting Books and earned <:enchanting:852069714511659058> ${earnedxp} Enchanting XP.`
			);
